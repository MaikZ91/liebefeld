...
const EventCard: React.FC<EventCardProps> = ({ event, onClick, className, compact = false, onLike }) => {
  ...
  if (compact) {
    return (
      <div 
        className={cn(
          "dark-glass-card rounded-lg p-1.5 cursor-pointer hover-scale mb-1.5 mx-0.5 w-[calc(100%-4px)]",
          className
        )}
        onClick={onClick}
      >
        <div className="flex justify-between items-start gap-1">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1 flex-wrap">
              {isNewEvent && (
                <Badge className="bg-green-600 text-white text-[0.65rem] flex items-center gap-0.5 h-3.5 px-1 py-0">
                  <BadgePlus className="w-2.5 h-2.5" />
                  <span>Neu</span>
                </Badge>
              )}
              {event.is_paid && (
                <Badge className="bg-amber-500 text-white text-[0.65rem] flex items-center gap-0.5 h-3.5 px-1 py-0">
              ...
              {event.link ? (
                ...
                >
                  {event.title}
                  <ExternalLink className="w-2.5 h-2.5 inline-flex flex-shrink-0" />
                </h4>
              ) : (
            ...
            <div className="flex flex-wrap items-center gap-1.5 mt-0.5 text-[0.65rem] text-white">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge className={cn(
              "flex-shrink-0 flex items-center gap-1 text-[0.65rem] font-medium whitespace-nowrap h-3.5 px-1 py-0",
            ...
            <div className="flex items-center gap-1.5">
              ...
              {displayLikes > 0 && (
                <span className="text-[0.65rem] text-white">{displayLikes}</span>
              )}
            </div>
          </div>
        </div>
