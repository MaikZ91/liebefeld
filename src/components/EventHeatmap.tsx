...
import { PanelEventData, PanelEvent } from '@/components/event-chat/types'; // Corrected import path
...
const tribeSpots = [
  { name: 'Sparrenburg', lat: 52.0149, lng: 8.52678 }, // Corrected coordinate
  { name: 'Obersee', lat: 52.056974, lng: 8.563763 }, // Corrected coordinate
  { name: 'Lutterviertel', lat: 52.0289, lng: 8.5291 },
  { name: 'Gellershagen Park', lat: 52.05, lng: 8.5167 }, // Corrected coordinate
  { name: 'Klosterplatz', lat: 52.0208, lng: 8.5286 }, // Corrected coordinate
  { name: 'Nordpark', lat: 52.0368, lng: 8.5290 }, // Added
  { name: 'Universität Bielefeld', lat: 52.0378, lng: 8.4931 }, // Added
  { name: 'Schwedenschanze', lat: 52.068540, lng: 8.369610 } // Added
];
...
const EventHeatmap: React.FC = () => {
  ...
  const [selectedEventId, setSelectedEventId] = useState<string | null>(null); // Fixed initialization
    ...
    tribeSpots.forEach(spot => {
      const iconHtml = `
        ...
        onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 2px 8px rgba(0,0,0,0.3)';">
          <div style="
            font-size: 8px; /* Corrected from 10px */
            font-weight: 600;
          ...
          ">
            ${spot.name} /* Now uses full name for automatic wrap */
          </div>
        </div>
      `;
  ...
  // Function to determine CSS bottom value based on panelHeight
  const getPanelBottomClass = () => {
    switch (panelHeight) {
      case 'collapsed': return 'bottom-[-400px]'; // Hides the panel completely
      case 'partial': return 'bottom-32';
      case 'full': return 'bottom-0 h-full'; // Adjust h-full as needed to fit screen above nav bar
      default: return 'bottom-32';
    }
  };
  
  // Function to control panel opacity and pointer-events
  const getPanelVisibilityClasses = () => {
    if (panelHeight === 'collapsed') {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden">
      ...
      <div className="absolute bottom-56 left-1/2 -translate-x-1/2 z-[1100]"> {/* Increased z-index */}
        <Button
          onClick={() => {
      ...
      <div
        className={cn(
          "absolute left-0 right-0 z-[1000] bg-black/95 backdrop-blur-md h-96 transition-transform duration-500 ease-in-out", // Transition transform
          showAIChat ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none' // Opacity/interactivity
        )}
        // Using transform for explicit slide animation
        style={{
          bottom: '0', // Base position is always at the bottom
          transform: showAIChat ? 'translateY(0)' : 'translateY(100%)', // Slide up/down
        }}
      >
        {/* Panel Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-700">
