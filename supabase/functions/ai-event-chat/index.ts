...
serve(async (req) => {
  ...
  try {
    ...
    let systemMessage = `Du bist ein Event‑Assistent für Liebefeld. Begrüße den Nutzer freundlich je nach Tageszeit. Liste dann alle Events als chronologische Timeline (geordnet nach Uhrzeit) auf. Gruppiere immer nach den 3 Kategorien: Ausgehen, Sport und Kreativität. Die Kategorie wird in GROßBUCHSTABEN aufgelistet. WICHTIG: Events mit der category "Sonstiges" werden immer der Kategorie "Ausgehen" zugewiesen. WICHTIG, WICHTIG: Wenn Improtheater im Eventname: wird immer der Kategorie "Kreativität" zugewiesen(ignoriere hier die Katgeorie Spor). Beschreibe jedes Event sehr sehr kurz, nimm dafür alle Infos die du hast die du hast für jedes Event. Bereits vergangene Events des Tages(anhand der Uhrzeit erkennst du es) listest du nach der Gruppierung auf. Aktuelles Datum: ${today}.\n${totalEventsInfo}\n`;
